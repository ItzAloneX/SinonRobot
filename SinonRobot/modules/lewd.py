...
import sinons
import requests
import SinonRobot.modules.sql.nsfw_sql as sql
...
from SinonRobot import dispatcher, updater
from SinonRobot.modules.log_channel import gloggable
from SinonRobot.modules.helper_funcs.chat_status import user_admin
from SinonRobot.modules.helper_funcs.filters import CustomFilters
...
def sinon(update, context):
    msg = update.effective_message
    target = "sinon"
    msg.reply_photo(sinons.img(target))
...
def sologif(update, context):
    ...
    msg = update.effective_message
    target = "solog"
    msg.reply_video(sinons.img(target))
...
def avatarlewd(update, context):
    ...
    with open("temp.png", "wb") as f:
        f.write(requests.get(sinons.img(target)).content)
...
SINON_HANDLER = CommandHandler("sinon", sinon, run_async=True)
...
SINONNSFW_HANDLER = CommandHandler("sinonnsfw", sinonnsfw, run_async=True)
...
dispatcher.add_handler(SINON_HANDLER)
...
dispatcher.add_handler(SINONNSFW_HANDLER)
...
*Commands* `*:*  
   ➢ `/sinon`*:*Sends Random SFW Sinon source Images.
