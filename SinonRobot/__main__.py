...
from SinonRobot import (ALLOW_EXCL, CERT_PATH, DONATION_LINK, LOGGER,
...
from SinonRobot.sinontext import PM_START_TEXT, SINON_DISPACHER_PIC, PM_PHOTO, SINON_N_IMG, TEXXT, SINON_IMG
...
buttons = [
    [
                        InlineKeyboardButton(
                             ...
                             url="https://t.me/SinonRobot?startgroup=true"),
...
For all command use /* [or](https://telegra.ph/file/2a100b2ec7707179b65f6.jpg) *!*
""".format(
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("SinonRobot.modules." +
                                              module_name)
...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_photo(
                random.choice(SINON_IMG), TEXXT.format(mention_html(update.effective_user.id, update.effective_user.first_name)),
...
@run_async
def sinon_callback_data(update, context):
    query = update.callback_query
    bot = context.bot
    uptime = get_readable_time((time.time() - StartTime))
    if query.data == "sinon_":
        query.message.edit_text(
            text=f"""Hello [{update.effective_user.first_name}](tg://user?id={update.effective_user.id}) I'm {context.bot.first_name}, a powerful group management bot built to help you manage your group easily.
                 ...
                 \n❍ Sinon is private source
                 \n❍ If you have any question about Sinon, let us know at [Sinon Support](t.me/{SUPPORT_CHAT}).""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            module = args[1].lower()
            update.effective_message.reply_photo(
            random.choice(SINON_N_IMG), caption= f"Oh Darling, Click the Button Below to get help of {module.capitalize()}",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        text="click here",
        ...
        update.effective_message.reply_photo(
            random.choice(SINON_N_IMG), caption= "Click the Button Below to get the list of possible commands.",
            reply_markup=InlineKeyboardMarkup(
                [
                  [
                  InlineKeyboardButton(text=" Click here", url="https://t.me/SinonRobot?start=help")
                  ]
                ]
            ),
        )
...
def get_settings(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            ...
            msg.reply_photo(
                random.choice(SINON_N_IMG), caption=text,
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(
                        text="Settings",
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.send_message(f"@{SUPPORT_CHAT}", f"[Sinon Is Online]({SINON_DISPACHER_PIC})", parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(
                [
                  [                  
                       InlineKeyboardButton(
                             text="💕 Add Sinon To Your Chat",
                             url="https://t.me/SinonRobot?startgroup=true")
                     ] 
                ]
            ),
            ...
            LOGGER.warning(
                "Sinon can't able to send message to support_chat, go and check!")
